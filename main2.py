...
def find_best_response(bot_name, user_input):
    """
    Find the best matching Assistant reply from bot's JSON.
    ✅ Only checks for exact cleaned match (no fuzzy logic).
    """
    ...
    # 🔎 Exact Match Check (cleaned)
    for i in range(0, len(conversations) - 1, 2):
        if not (
            conversations[i]["role"] == "User"
            and conversations[i+1]["role"] == "Assistant"
        ):
            continue

        conv_content_clean = normalize_text(conversations[i].get("content", ""))
        if user_input_clean == conv_content_clean:
            return conversations[i+1].get("content")

    # ❌ No match found → let caller handle fallback (Markov / fake response)
    return None
